...
train_data<-cbind(train_data, m)
train_data<-train_data[,1:16]


test_pca1<-predict(pca_fun,newdata=bs_pvaTest)
...
#Ridge Regression
RR_model<-cv.glmnet(data.matrix(train_data %>% select(-TARGET_B)), train_data$TARGET_B, family="binomial")
...
sc_Tst<-predict(RR_model, data.matrix(select(test_pca,  -"TARGET_B")), s="lambda.1se")
auc(as.numeric(as.character(test_pca$TARGET_B)), sc_Tst)
x<-as.matrix(train_data[,-1])
y<-as.double(as.matrix(train_data[,1]))
set.seed(999)

#-------------------------------LASSO--------------------------------------------
cv.lasso<-cv.glmnet(x,y,family='binomial',alpha=0,parallel=TRUE,standardize=TRUE,type.measure='auc')

#Performance with lambda 1se
sc_Trn_lasso_1se<-predict(cv.lasso, data.matrix(select(train_data, -"TARGET_B",)), s="lambda.1se")

sc_Trn_lasso_1se[sc_Trn_lasso_1se>0.5]  <- 1
sc_Trn_lasso_1se[sc_Trn_lasso_1se<=0.5] <- 0
...
sc_Trn_lasso_min<-predict(cv.lasso, data.matrix(select(train_data, -"TARGET_B",)), s="lambda.min")

sc_Trn_lasso_min[sc_Trn_lasso_min>0.5]  <- 1
sc_Trn_lasso_min[sc_Trn_lasso_min<=0.5] <- 0
...
sc_Tst_lasso_1se<-predict(cv.lasso, data.matrix(select(test_pca1,  -"TARGET_B")), s="lambda.1se")

sc_Tst_lasso_1se[sc_Tst_lasso_1se>0.5]  <- 1
sc_Tst_lasso_1se[sc_Tst_lasso_1se<=0.5] <- 0
sc_Tst_lasso_1se  <- as.factor(sc_Tst_lasso_1se)

confusionMatrix(sc_Tst_lasso_1se,test_pca1$TARGET_B)

#Test Performance with lambda.min
sc_Tst_lasso_min<-predict(cv.lasso, data.matrix(select(test_pca1,  -"TARGET_B")), s="lambda.min")

sc_Tst_lasso_min[sc_Tst_lasso_min>0.5]  <- 1
sc_Tst_lasso_min[sc_Tst_lasso_min<=0.5] <- 0
...
cv.ridge<-cv.glmnet(x,y,family='binomial',alpha=1,parallel=TRUE,standardize=TRUE,type.measure='auc')
plot(cv.ridge)

#Performance with lambda 1se
sc_Trn_ridge_1se<-predict(cv.ridge, data.matrix(select(train_data, -"TARGET_B",)), s="lambda.1se")

sc_Trn_ridge_1se[sc_Trn_ridge_1se>0.5]  <- 1
sc_Trn_ridge_1se[sc_Trn_ridge_1se<=0.5] <- 0
sc_Trn_ridge_1se  <- as.factor(sc_Trn_ridge_1se)
confusionMatrix(sc_Trn_ridge_1se,train_data$TARGET_B)

#Performance with lambda min
sc_Trn_ridge_min<-predict(cv.ridge, data.matrix(select(train_data, -"TARGET_B",)), s="lambda.min")

sc_Trn_ridge_min[sc_Trn_ridge_min>0.5]  <- 1
sc_Trn_ridge_min[sc_Trn_ridge_min<=0.5] <- 0
sc_Trn_ridge_min  <- as.factor(sc_Trn_ridge_min)
confusionMatrix(sc_Trn_ridge_min,train_data$TARGET_B)

#Test Performance with lambda.1se
sc_Tst_ridge_1se<-predict(cv.ridge, data.matrix(select(test_pca1,  -"TARGET_B")), s="lambda.1se")

sc_Tst_ridge_1se[sc_Tst_ridge_1se>0.5]  <- 1
sc_Tst_ridge_1se[sc_Tst_ridge_1se<=0.5] <- 0
sc_Tst_ridge_1se  <- as.factor(sc_Tst_ridge_1se)
confusionMatrix(sc_Tst_ridge_1se,test_pca1$TARGET_B)

#Test Performance with lambda.min
sc_Tst_ridge_min<-predict(cv.ridge, data.matrix(select(test_pca1,  -"TARGET_B")), s="lambda.min")

sc_Tst_ridge_min[sc_Tst_ridge_min>0.5]  <- 1
sc_Tst_ridge_min[sc_Tst_ridge_min<=0.5] <- 0
sc_Tst_ridge_min  <- as.factor(sc_Tst_ridge_min)
confusionMatrix(sc_Tst_ridge_min,test_pca1$TARGET_B)

#---------------------------------With oversampling=50%---------------------------

bs_pvaTrainos <- ovun.sample(TARGET_B ~., data=as.data.frame(pvaTrain), na.action = na.pass, method = "both", p=0.5)$data
mos<-bs_pvaTrainos$TARGET_B
bs_pvaTrainos %>% group_by(TARGET_B) %>% count()

bs_pvaTrainos$TARGET_B<-as.factor(bs_pvaTrainos$TARGET_B)

bs_pvaTestos <- ovun.sample(TARGET_B ~., data=as.data.frame(pvaTest), na.action = na.pass, method = "both", p=0.5)$data
bs_pvaTestos %>% group_by(TARGET_B) %>% count()

#-----------------------------------------Randomforest-----------------------------------------------------------------------------------------


rf_mos <- ranger(TARGET_B ~., data=bs_pvaTrainos, num.trees=200, probability = TRUE, importance='permutation')
     #probability=TRUE will give us predicted prob values form the predict(..) function
    # with pobability=FALSE (default), the predict(..) function will give the predicted class label only

abcos<-rf_mos$variable.importance %>% sort(decreasing =TRUE)
qos<-head(abcos,30)
rfvarimpos<-names(qos)
...
varlistos<-union(rfvarimpos, gbmvarimp)
...
#view(pnn)
bs_pvaTrainos <- bs_pvaTrainos %>% select(varlistos)

pca_funos<-prcomp(bs_pvaTrainos,scale=T)
std_devos<-pca_funos$sdev
pr_varos<-std_devos^2

prop_varexos<-pr_varos/sum(pr_varos)
plot(prop_varexos)
view(pca_fun$varos)

prop_varexos[1:20]
plot(prop_varexos,xlab="Principal Component", ylab="Proportion of variance explained", type="b")

plot(cumsum(prop_varexos),xlab="Principal component",ylab="Cumulative proprtion of variance explained", type="b")

fviz_screeplot(pca_funos, ncp=50)

train_dataos<-data.frame(TARGET_B=bs_pvaTrain$TARGET_B,pca_fun$x)
train_dataos<-cbind(train_dataos, m)
train_dataos<-train_dataos[,1:16]


test_pca1os<-predict(pca_funos,newdata=bs_pvaTestos)
test_pca1os<-as.data.frame(test_pca1os)
test_pca1os<-data.frame(TARGET_B=bs_pvaTestos$TARGET_B,test_pca1os)
test_pca1os<-test_pca1os[,1:16]


#Ridge Regression
RR_modelos<-cv.glmnet(data.matrix(train_dataos %>% select(-TARGET_B)), train_dataos$TARGET_B, family="binomial")

plot(RR_model)

coef(RR_modelos, s="lambda.1se") %>% tidy() %>% view()
coef(RR_modelos, s="lambda.min") %>% tidy() %>% view()
RR_modelos$lambda.1se
RR_modelos$lambda.min

xos<-as.matrix(train_dataos[,-1])
yos<-as.double(as.matrix(train_dataos[,1]))
set.seed(999)

#-------------------------------LASSO--------------------------------------------
cv.lassoos<-cv.glmnet(xos,yos,family='binomial',alpha=0,parallel=TRUE,standardize=TRUE,type.measure='auc')

#Performance with lambda 1se
sc_Trn_lasso_1seos<-predict(cv.lassoos, data.matrix(select(train_dataos, -"TARGET_B",)), s="lambda.1se")

sc_Trn_lasso_1seos[sc_Trn_lasso_1seos>0.5]  <- 1
sc_Trn_lasso_1seos[sc_Trn_lasso_1seos<=0.5] <- 0
sc_Trn_lasso_1seos  <- as.factor(sc_Trn_lasso_1seos)

confusionMatrix(sc_Trn_lasso_1seos,train_dataos$TARGET_B)

#Performance with lambda.min
sc_Trn_lasso_minos<-predict(cv.lassoos, data.matrix(select(train_dataos, -"TARGET_B",)), s="lambda.min")

sc_Trn_lasso_minos[sc_Trn_lasso_minos>0.5]  <- 1
sc_Trn_lasso_minos[sc_Trn_lasso_minos<=0.5] <- 0
sc_Trn_lasso_minos <- as.factor(sc_Trn_lasso_minos)

confusionMatrix(sc_Trn_lasso_minos,train_dataos$TARGET_B)

#Test Performance with lambda.1se
sc_Tst_lasso_1seos<-predict(cv.lassoos, data.matrix(select(test_pca1os,  -"TARGET_B")), s="lambda.1se")

sc_Tst_lasso_1seos[sc_Tst_lasso_1seos>0.5]  <- 1
sc_Tst_lasso_1seos[sc_Tst_lasso_1seos<=0.5] <- 0
sc_Tst_lasso_1seos  <- as.factor(sc_Tst_lasso_1seos)

confusionMatrix(sc_Tst_lasso_1seos,test_pca1os$TARGET_B)

#Test Performance with lambda.min
sc_Tst_lasso_minos<-predict(cv.lassoos, data.matrix(select(test_pca1os,  -"TARGET_B")), s="lambda.min")

sc_Tst_lasso_minos[sc_Tst_lasso_minos>0.5]  <- 1
sc_Tst_lasso_minos[sc_Tst_lasso_minos<=0.5] <- 0
sc_Tst_lasso_minos  <- as.factor(sc_Tst_lasso_minos)

confusionMatrix(sc_Tst_lasso_minos,test_pca1os$TARGET_B)

#---------------------------Ridge--------------------------------------------------

cv.ridgeos<-cv.glmnet(xos,yos,family='binomial',alpha=1,parallel=TRUE,standardize=TRUE,type.measure='auc')
plot(cv.ridgeos)

#Performance with lambda 1se
sc_Trn_ridge_1seos<-predict(cv.ridgeos, data.matrix(select(train_dataos, -"TARGET_B",)), s="lambda.1se")

sc_Trn_ridge_1seos[sc_Trn_ridge_1seos>0.5]  <- 1
sc_Trn_ridge_1seos[sc_Trn_ridge_1seos<=0.5] <- 0
sc_Trn_ridge_1seos  <- as.factor(sc_Trn_ridge_1seos)
confusionMatrix(sc_Trn_ridge_1seos,train_dataos$TARGET_B)

#Performance with lambda min
sc_Trn_ridge_minos<-predict(cv.ridgeos, data.matrix(select(train_dataos, -"TARGET_B",)), s="lambda.min")

sc_Trn_ridge_minos[sc_Trn_ridge_minos>0.5]  <- 1
sc_Trn_ridge_minos[sc_Trn_ridge_minos<=0.5] <- 0
sc_Trn_ridge_minos  <- as.factor(sc_Trn_ridge_minos)
confusionMatrix(sc_Trn_ridge_minos,train_dataos$TARGET_B)

#Test Performance with lambda.1se
sc_Tst_ridge_1seos<-predict(cv.ridgeos, data.matrix(select(test_pca1os,  -"TARGET_B")), s="lambda.1se")

sc_Tst_ridge_1seos[sc_Tst_ridge_1seos>0.5]  <- 1
sc_Tst_ridge_1seos[sc_Tst_ridge_1seos<=0.5] <- 0
sc_Tst_ridge_1seos  <- as.factor(sc_Tst_ridge_1seos)
confusionMatrix(sc_Tst_ridge_1seos,test_pca1os$TARGET_B)

#Test Performance with lambda.min
sc_Tst_ridge_minos<-predict(cv.ridgeos, data.matrix(select(test_pca1os,  -"TARGET_B")), s="lambda.min")

sc_Tst_ridge_minos[sc_Tst_ridge_minos>0.5]  <- 1
sc_Tst_ridge_minos[sc_Tst_ridge_minos<=0.5] <- 0
sc_Tst_ridge_minos  <- as.factor(sc_Tst_ridge_minos)
confusionMatrix(sc_Tst_ridge_minos,test_pca1os$TARGET_B)
